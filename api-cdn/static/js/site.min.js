...
$jscomp.polyfill("Array.prototype.find",function(a){return a?a:function(a,c){return $jscomp.findInternal(this,a,c).v}},"es6-impl","es3");var userSettings={},newPosts={},maxPosts=10,seenPosts=[],userID,postIndex=0,userFilters=[];$(function(){userID=$("#userID").val();AddTagVoteListener();AddPostVoteListener();AddMenuHandler();AddFilterSearch();GetUserSettings();LoadKeybinds();AddToSeenPosts();AddFilterHandler();LoadUserFilters();Drag2();FilterToggle();Recaptcha();AddInfoBar();HookSubClick()});
function HookSubClick(){$(".filterbar-subscribe").click(function(a){a.preventDefault();a=$(a.currentTarget);var b=a.children("span");b.hasClass("ti-minus")?(b.removeClass("ti-minus"),b.addClass("ti-plus")):(b.removeClass("ti-plus"),b.addClass("ti-minus"));a=a.attr("data-filterid");console.log(a);void 0!=a&&$.get("/api/filter/"+a+"/sub",function(a){console.log(a)})})}
function LoadUserFilters(){$("#userID").val()?$.get("/api/user/filters",function(a){console.log(a);0==a.error&&(userFilters=a.data)}):console.log("couldnt get userID")}function AddInfoBar(){$(".infobar-title").click(function(a){$(".infobar-body").toggle();a.preventDefault()})}function Recaptcha(){$(".form-login").focusin(function(){$(".form-login > div").show()});$(".form-login").focusout(function(){$(".form-login > div").hide()})}
function Drag2(){console.log("this");interact(".post").draggable({inertia:!0,onmove:dragMoveListener,onend:onEndListener,onstart:onStartListener,axis:"x"})}function onStartListener(a){$(a.target).children("a").click(function(a){a.preventDefault()});console.log("this started")}
...
a.preventDefault()})}function Upvote(a){a=$(":focus");a.length&&VotePost(a,"up")}function Downvote(a){a=$(":focus");console.log(a);a.length&&(console.log(2),VotePost(a,"down"))}function AddPostVoteListener(){$(".post-upvote").click(function(a){a.preventDefault();VotePost($(this).parent().parent(),"up")});$(".post-downvote").click(function(a){a.preventDefault();VotePost($(this).parent().parent(),"down")})}
...
function MoveFocus(a){a.preventDefault();var b=$(":focus");if(b.hasClass("post")){var c;"ArrowUp"==a.key?c=b.prev():"ArrowDown"==a.key?c=b.next():console.log(a.key);c.length&&c.focus()}}function LoadKeybinds(){Mousetrap.bind("up",MoveFocus);Mousetrap.bind("down",MoveFocus);Mousetrap.bind("enter",OpenLink);Mousetrap.bind("space",OpenComments);Mousetrap.bind("right",Upvote);Mousetrap.bind("left",Downvote);$("#posts").children().first().focus()}userFilters={};
function GetUserSettings(){var a=$("#userID").val();a&&$.getJSON("/api/user/"+a+"/settings",function(a){"success"==a.status&&(userSettings=a.data)})}function ChangeFocus(a){index+=a;a=$("#posts").children().length-1;index=Math.max(index,0);index=Math.min(index,a);$("#posts").children().eq(index).focus()}function UserHasFilter(a){var b=null;$.each(userFilters,function(c,d){d.id==a&&(console.log("found"),b=!0)});return b}
...
function AddMenuHandler(){$(".settings-link").click(function(a){var b=$(".settings-menu");b.toggle();b.focus();b.focusout(function(a){$(a.relatedTarget).parents(".settings-menu").length||b.hide()});a.preventDefault()})}function GetFreshPost(){for(var a=newPosts.shift();-1!=$.inArray(a.id,seenPosts);)if(a=newPosts.shift(),void 0==a&&(postIndex+=100,LoadNewPosts(postIndex,postIndex+100),a=newPosts.shift(),void 0==a))return;return a}
