...
$jscomp.polyfill("Array.prototype.find",function(a){return a?a:function(a,c){return $jscomp.findInternal(this,a,c).v}},"es6-impl","es3");var userSettings={},newPosts={},maxPosts=10,seenPosts=[],userID,postIndex=0;$(function(){userID=$("#userID").val();AddTagVoteListener();AddPostVoteListener();AddMenuHandler();AddFilterSearch();GetUserSettings();LoadKeybinds();LoadNewPosts();AddToSeenPosts();AddFilterHandler();Drag2();FilterToggle();Recaptcha();$(".settings-menu").focutout(function(){$(".settings-menu").hide()})});
function Recaptcha(){$(".form-login").focusin(function(){$(".form-login > div").show()});$(".form-login").focusout(function(){$(".form-login > div").hide()})}function Drag2(){console.log("this");interact(".post").draggable({inertia:!0,onmove:dragMoveListener,onend:onEndListener,onstart:onStartListener,axis:"x",restrict:{restriction:"parent",endOnly:!0,elementRect:{top:0,left:0,bottom:1,right:1}}})}
function onStartListener(a){$(a.target).children("a").click(function(a){a.preventDefault()});console.log("this started")}
function onEndListener(a){var b=a.target,c=(parseFloat(b.getAttribute("data-x"))||0)+a.dx;$(a.target).children("a").off("click");a.preventDefault();console.log(c,100>c,typeof c);console.log(a);0<=c&&-200<c?VotePost($(a.target),"down"):0<c&&200<c?VotePost($(a.target),"up"):(b.style.webkitTransform=b.style.transform="translate(0px, 0px)",b.setAttribute("data-x",0),b.setAttribute("data-y",0))}
...
function FilterToggle(){var a=$(".hamburger");a.on("click",function(b){a.toggleClass("is-active");console.log("that");$(".filter-bar").toggle()});$(".toggle-filterstyle").click(function(a){$(".filter-styles").toggle();a.preventDefault()})}function DraggablePosts(){$(".post").draggable({axis:"x",stop:function(a,b){100<b.position.left?VotePost(this,"up"):-100>b.position.left&&VotePost(this,"down");$(b.helper).animate({left:"0px"},200)}})}function Upvote(a){a=$(":focus");a.length&&VotePost(a,"up")}
function Downvote(a){a=$(":focus");console.log(a);a.length&&(console.log(2),VotePost(a,"down"))}function AddPostVoteListener(){$(".postUpvote").click(function(a){a.preventDefault();VotePost($(this).parent().parent(),"up")});$(".postDownvote").click(function(a){a.preventDefault();VotePost($(this).parent().parent(),"down")})}
function VotePost(a,b){var c=$(a).children(".postID").val(),d=$(a).children(".postHash").val();"1"==userSettings.hideVotedPosts&&(-1==$.inArray(c,seenPosts)&&seenPosts.push(c),LoadMorePosts($(a).parents(".post")),$(a).hide("slide",{direction:"up"==b&&"right"||"left"},200,function(){var b=$(a).next();console.log(b);b.length&&b.focus();$(a).remove()}));"up"==b?($(a).css("border","solid 1px green"),c="/api/post/"+c+"/upvote?hash="+d):($(a).css("border","solid 1px red"),c="/api/post/"+c+"/downvote?hash="+
d);$.get(c,function(a){})}function AddFilterHandler(){}function AddToSeenPosts(){$.each($("#posts").children(),function(a,b){var c=$(b).find(".postID").val();seenPosts.push(c)});console.log(seenPosts)}function LoadNewPosts(a,b){$.getJSON("/api/frontpage?startat=1&endat=100",function(a){console.log(a);"success"==a.status&&(console.log(a.data),newPosts=a.data,"undefined"==a.data&&(newPosts=[]),console.log(newPosts.length+" new posts got from server"))})}
...
function GetUserSettings(){var a=$("#userID").val();a&&$.getJSON("/api/user/"+a+"/settings",function(a){console.log(a);"success"==a.status&&(userSettings=a.data)})}function ChangeFocus(a){index+=a;a=$("#posts").children().length-1;index=Math.max(index,0);index=Math.min(index,a);$("#posts").children().eq(index).focus()}
...
function AddFilterSearch(){console.log("adding this");$(".filter-search-form").submit(function(a){a.preventDefault()});$("#filterSearch").on("input",function(a){a.preventDefault();clearTimeout($(this).data("timeout"));var b=this;$(this).data("timeout",setTimeout(function(){console.log("searching");b.value.trim()?$.get("/api/filter/search/"+b.value,{search:b.value},UpdateSidebar):$.get("/api/user/filters",{search:b.value},UpdateSidebar)},200))})}
function AddMenuHandler(){$(".settings-link a").click(function(a){$(".settings-menu").focus();$(".settings-menu").toggle();a.preventDefault()});$("#box-two").hide();$("#infoBoxLink").click(function(a){a.preventDefault();$("#box-one").show();$("#box-two").hide()});$("#filterBoxLink").click(function(a){a.preventDefault();$("#box-one").hide();$("#box-two").show()})}
function GetFreshPost(){for(var a=newPosts.shift();-1!=$.inArray(a.id,seenPosts);)if(a=newPosts.shift(),void 0==a&&(postIndex+=100,LoadNewPosts(postIndex,postIndex+100),a=newPosts.shift(),void 0==a))return;return a}
